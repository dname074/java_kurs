...
public class Exercises {
    ...
    public static void main(String[] args) {
        ...
        List<String> holdingNames = getHoldingNames();
        System.out.println(holdingNames);
        ...
        System.out.println(getAccountAmountInPLN(new Account(AccountType.ROR1, "54325", new BigDecimal("5.5"), Currency.EUR)));

        System.out.println(getUsersForPredicate(user -> user.getFirstName().equals("Kasia")));

        System.out.println(getUserNames());

        showAllUser();

        User rich = getRichestWoman().orElseThrow(() -> new IllegalArgumentException("Nie znaleziono"));
        ...
        System.out.println(getUser(user -> user.getFirstName().equals("Kasia")));

        System.out.println(createAccountsMap());
    }

    /**
     * Napisz metodę, która zwróci liczbę holdingów, w których jest przynajmniej jedna firma. Ready
     */
    ...
    public static User getUser(final Predicate<User> predicate) {
        return getUserStream()
                .filter(predicate)
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Nie znaleziono takiego uzytkownika"));
    }
    ...
    public static void showAllUser() {
        getUserStream()
                .sorted(Comparator.comparing(User::getFirstName, String.CASE_INSENSITIVE_ORDER.reversed())
                        .thenComparing(User::getLastName, String.CASE_INSENSITIVE_ORDER.reversed()))
                .forEach(user ->
